...
movediver(int n)
{
  ...
  { // Either we're in dive, or we've past it and are back at top.
...
movesniper(int n)
{
  ...
  {                      // Note: This working correctly is dependent on there always being a tick of tonguelength == 0
...
void
drawsprites(void) /*  everything drawn here needs to be cleared at clearsprites()  */
{
  int n;
...
void
algorithmicenemynumbers(int thislevel) // Set enemy numbers algorithmically. Only called if numbers not predefined above.
{
  assert(thislevel > 0);
  ...
  if (thislevel % 2 == 0)
  {
    ...
    if (levelinfo.enemycount[BROWNIAN] > 12)
      levelinfo.enemycount[BROWNIAN] = 12;
  }
  levelinfo.enemycount[ROAMER] = ((thislevel - 2) / 9) + 1 - levelinfo.enemycount[BROWNIAN];
  if (levelinfo.enemycount[ROAMER] < 0)
    levelinfo.enemycount[ROAMER] = 0;
  if (levelinfo.enemycount[ROAMER] > 12)
    levelinfo.enemycount[ROAMER] = 12;
  if (thislevel % 3 == 0)
    levelinfo.enemycount[LASERGUN] = 1;
  if (thislevel % 7 == 0)
    levelinfo.enemycount[SKULL] = 1;
  if (thislevel % 4 == 0)
    levelinfo.enemycount[DIVER] = thislevel / 4;
  if (levelinfo.enemycount[DIVER] > 5)
    levelinfo.enemycount[DIVER] = 5;
  if (thislevel % 4 == 1)
    levelinfo.enemycount[GUNNER] = 2;
  if (thislevel % 5 == 4)
    levelinfo.enemycount[BOUNCER] = (thislevel / 10) + 1;
  if (thislevel % 3 == 1 || thislevel % 7 == 1)
    levelinfo.enemycount[EYEBALL] = 1;
  if (thislevel % 8 == 3)
    levelinfo.enemycount[DROPPER] = 3;
  if (thislevel % 7 == 1)
  ...
  if (thislevel % 4 == 2)
  {
    ...
    if (levelinfo.enemycount[BOMBER] > 5)
      levelinfo.enemycount[BOMBER] = 5;
  }
...
void
makelevel(void) // Set the various objects into existence.
...

{
  ...
  for (n = 0; n < levelinfo.coins && n < MAX_COINS; n++)
  {
    ...
    if (fabs(coin[n].speedx) < 0.5)
      coin[n].speedx = sign(coin[n].speedx) * 0.5;
    coin[n].speedy = rnd() * 6 - 3;
    if (fabs(coin[n].speedy) < 0.5)
      coin[n].speedy = sign(coin[n].speedy) * 0.5;
  }
  ...
  for (n = 0; n < levelinfo.diamonds && n < MAX_DIAMONDS; n++)
  {
    ...
    if (fabs(diamond[n].speedx) < 0.5)
      diamond[n].speedx = sign(diamond[n].speedx) * 0.5;
    if (levelinfo.topdiamonds == 0)
    {
      diamond[n].y = (rnd() * (START_LIGHTNING_Y - (SPRITE_SIZE * 2))) + SPRITE_SIZE;
      diamond[n].speedy = rnd() * 6 - 3;
      if (fabs(diamond[n].speedy) < 0.5)
        diamond[n].speedy = sign(diamond[n].speedy) * 0.5;
    }
    else
    {
      diamond[n].y = SPRITE_SIZE / 2;
...
int
playerdeath(void) // Return 1 if player is dead.
{
  int komideadflag = 0;
  int n;
...
int
lightningdeath(void) // This will be called from several places for aesthetic reasons.
...
int
electraboltsdeath(void) // Is there a direct collision between Komi and any electra bolts?
{
  int leftelectrax;
  int rightelectrax;
  int n;
...
void
cleanexit(int exitstatus) // Exit cleanly, shutting down SDL.
{
  if (nosound == 0)
  {
    Mix_CloseAudio();
...
void
loadsound(Mix_Chunk** thesound, char* directory, char* filename)
{
...
void
playsound(Mix_Chunk* thesound) // Pass sound pointer to Mix_PlayChannel() assuming sound is on.
{
  if (nosound == 0)
  {
...
void
drawmenu(int highlight_start, int highlight_quit, int highlight_shuffle)
...
void
drawspeedrect(void) // The rect showing the speed at the bottom left of the menu.
{
...
void
updatemousemap(void) // Keeps track of whether the mouse button is down, and what position it clicked on.
{
...
void
manageevents(void) // Manage relevant keyboard, mouse, and quit events.
{
...
int
leveldone(void) // Return 1 if there are no coins or diamonds on the level any more.
{
  int n;
...
int
sign(float value) // Returns sign of value, either -1 for negative, 1 for positive, or 0 for zero.
{
  if (value < 0)
    return -1;
  if (value > 0)
...
float
rnd(void) // Random number between 0 and 1.
{
...
void
fadeout(void) // Not so much a fade as a series of black lines...
...

{
  ...
  if (SDL_MUSTLOCK(virtue))
    SDL_LockSurface(virtue); // Use of line() requires this.
...
void
screenshot(SDL_Surface* surface, char* directory, char* filename)
{
  char fullpath[TEXTBUFFERSIZE];
...
void
drawelectricity(int x1, int y, int x2, int electraflag) // Draw electricity between (x1, y) and (x2, y)
...
void
drawskullpull(int clearflag, int n) // Draw the circles from the Skull (enemy n) to Komi.
{
...
void
goodieaction(int type) // This function should play some sound, be it the standard eat sound or something else.
{
  switch (type)
  {
    case EXTRALIFE:
...
void
addcornershots(void) // Add 4 inbound friendly shots - one at each corner. Used if we get the "destructor" powerup.
{
  int n;
...
void
addkomishot(void) // Add an upward travelling friendly shot fired from Komi's position.
{
  int n;
...
int
intrnd(int max) // Return integer between 0 and max inclusive.
{
  int result;
...
void
addscore(int num) // Also checks if we're over the points required to get a new life.
{
  int initialscore;
...
void
saveprefs(char* directory, char* filename)
{
  char fullpath[TEXTBUFFERSIZE];
...
void
loadprefs(char* directory, char* filename)
{
  char fullpath[TEXTBUFFERSIZE];
...
int
playmusic(char* directory, char* filename, int loops)
{
  char fullpath[TEXTBUFFERSIZE];
  FILE* testopen;
...
void
setprefsdir(void) // Try to get a sensible directory to save prefs to.
